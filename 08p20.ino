...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    if (dist_raw < 200.0){
      analogWrite(PIN_LED,225.0*((200 - dist_raw) / 100)); //duty cycle = (200.0 - dist_raw) / 100.0
    }
    else if (dist_raw == 200.0){
      analogWrite(PIN_LED,0);
    }
    else {    //dist_raw가 200이상일 경우
      analogWrite(PIN_LED,225.0*((dist_raw - 200) / 100)); //duty cycle = (dist_raw - 200.0) / 100.0
    }
  }
 // Assume that it takes 5ms to do something.
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if (reading == 0.0){
    return dist_raw; //reading값이 0일 경우 바로 직전에 측정한 거리값을 반환해준다
  }
  else {
    return reading;
  }
